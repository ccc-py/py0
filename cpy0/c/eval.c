...
PyValue *eval_expr(PyRuntime *rt, PyEnv *env, AstNode *node) {
    ...
    switch (node->kind) {
        ...
        case AST_ATTRIBUTE: {
            ...
            if (value->type == PY_SYS && strcmp(node->as.attribute.attr, "argv") == 0) {
                return value->as.sys->argv;
            }
        ...
        case AST_SUBSCRIPT: {
            ...
            if (index->type != PY_INT) {
                die("subscript index must be int");
            }
            if (value->type == PY_LIST) {
                return py_list_get(value, index->as.i);
            }
            if (value->type == PY_STR) {
                long i = index->as.i;
                size_t len = strlen(value->as.s);
                if (i < 0 || (size_t)i >= len) {
                    die("string index out of range");
                }
                char text[2];
                text[0] = value->as.s[i];
                text[1] = '\0';
